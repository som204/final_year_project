...
import { LayoutDashboard, BookPlus, UserPlus, FolderKanban, FileText, ChartLine, GraduationCap, LogOut, Menu, X } from 'lucide-react';
  ...
  return (
    <aside className="w-full md:w-64 bg-slate-900 text-white flex flex-col shrink-0 md:h-full relative z-40">
      <div className="p-4 md:p-6 border-b border-slate-800 flex justify-between items-center">
        <h2 className="text-xl md:text-2xl font-semibold m-0">Institute Admin</h2>
        <button 
      ...
      <div className={`${isOpen ? 'flex' : 'hidden'} md:flex flex-col absolute md:relative top-full md:top-auto left-0 md:left-auto w-full md:w-auto bg-slate-900 md:grow md:h-auto h-[calc(100vh-73px)] border-b border-slate-800 md:border-0`}>
        <nav className="flex flex-col gap-2 p-4 grow overflow-y-auto">
          <NavLink to="/institute-admin/dashboard" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <LayoutDashboard size={20} className="shrink-0" />
            <span>Dashboard</span>
          </NavLink>
          <NavLink to="/institute-admin/department" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <BookPlus size={20} className="shrink-0" />
            <span>Department</span>
          </NavLink>
          <NavLink to="/institute-admin/faculty" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <UserPlus size={20} className="shrink-0" />
            <span>Faculty</span>
          </NavLink>
          <NavLink to="/institute-admin/students" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <GraduationCap size={20} className="shrink-0" />
            <span>Students</span>
          </NavLink>
          <NavLink to="/institute-admin/project" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <FolderKanban size={20} className="shrink-0" />
            <span>Project</span>
          </NavLink>
          <NavLink to="/institute-admin/report" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <FileText size={20} className="shrink-0" />
            <span>Report</span>
          </NavLink>
          <NavLink to="/institute-admin/analyse" className={navLinkClass} onClick={() => setIsOpen(false)}>
            <ChartLine size={20} className="shrink-0" />
            <span>Analyse</span>
          </NavLink>
          
          <div className="mt-auto md:mt-0 pt-4 md:pt-0">
             <NotificationBell to="/institute-admin/notifications" onClick={() => setIsOpen(false)} />
          </div>
        </nav>
